...
class ViewController: UIViewController {
    ...
    @IBAction func choosedChannel(sender: UISegmentedControl) {
        channel = channelsList[sender.selectedSegmentIndex];
        update()
    }
    
    @IBOutlet weak var selector_o: UISegmentedControl!
    ...
    func update(){
        ...
        for (var i = 0;i<4;i++){
            selector_o.setTitle(channelsTitleList[i], forSegmentAtIndex: i);
